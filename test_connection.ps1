...
# Tests connectivity between your PC (172.20.10.8) and Asterisk (172.20.10.14)

Write-Host "=== VoIP Connection Test ===" -ForegroundColor Green
Write-Host "PC IP: 172.20.10.8" -ForegroundColor Cyan
Write-Host "Asterisk IP: 172.20.10.14" -ForegroundColor Cyan
...
try {
    $config = Invoke-RestMethod -Uri "http://172.20.10.8:8080/config" -Method GET -TimeoutSec 5
...
# Test AMI port (5038)
Write-Host "   Testing AMI port (172.20.10.14:5038)..." -NoNewline
try {
    $tcpClient = New-Object System.Net.Sockets.TcpClient
    $tcpClient.Connect("172.20.10.14", 5038)
    $tcpClient.Close()
    Write-Host " ✓ OPEN" -ForegroundColor Green
} catch {
    Write-Host " ✗ CLOSED" -ForegroundColor Red
    Write-Host "   Check if Asterisk is running on 172.20.10.14" -ForegroundColor Yellow
}

# Test WebSocket port (8088)
Write-Host "   Testing WebSocket port (172.20.10.14:8088)..." -NoNewline
try {
    $tcpClient = New-Object System.Net.Sockets.TcpClient
    $tcpClient.Connect("172.20.10.14", 8088)
    $tcpClient.Close()
    Write-Host " ✓ OPEN" -ForegroundColor Green
} catch {
    Write-Host " ✗ CLOSED" -ForegroundColor Red
...
if (Test-Path ".env") {
    ...
    if ($apiUrl -eq "http://172.20.10.8:8080" -and $sipServer -eq "172.20.10.14") {
...
Write-Host "• If Asterisk ports closed: Check Asterisk server at 172.20.10.14" -ForegroundColor White
